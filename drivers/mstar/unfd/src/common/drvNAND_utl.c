...
U32 drvNAND_SearchCIS(void)
{
	...
	for (u16_PBA = 0; u16_PBA < 0x40; u16_PBA++)
	{
		...
		#else
		// read twice to avoid a very strange bug which will not ve solve
...
void dump_part_info(PARTITION_INFO_t *pPartInfo)
{
	...
	nand_debug(UNFD_DEBUG_LEVEL, 1, "#        PART INFO                            #\n");
	nand_debug(UNFD_DEBUG_LEVEL, 1, "###############################################\n");

	nand_debug(UNFD_DEBUG_LEVEL, 1, "pPartInfo: 0x%08lx\n", (U32)pPartInfo);
	nand_debug(UNFD_DEBUG_LEVEL, 1, "u32_ChkSum      : 0x%04lx\n", pPartInfo->u32_ChkSum);
//	nand_debug(UNFD_DEBUG_LEVEL, 1, "u32_BLSectorCnt : 0x%04lx\n", pPartInfo->u32_BLSectorCnt);
//	nand_debug(UNFD_DEBUG_LEVEL, 1, "u32_OSSectorCnt : 0x%04lx\n", pPartInfo->u32_OSSectorCnt);
	nand_debug(UNFD_DEBUG_LEVEL, 1, "u16_PartCnt     : 0x%04x\n", pPartInfo->u16_PartCnt);
	nand_debug(UNFD_DEBUG_LEVEL, 1, "u16_UnitByteCnt : 0x%04x\n", pPartInfo->u16_UnitByteCnt);
